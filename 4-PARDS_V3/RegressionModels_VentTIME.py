# ======================================================================================
# TBME Major Revision Utilities (REGRESSION: VentTIME feature set; NO PCA)
...
save_root = os.path.join(out_root_models, "SavedModels_VentTIME_8")  # NEW folder
...
# ---------------------------
# VentTIME feature set per TW (TW1..TW6)
# ---------------------------
base_template = [
...
if RUN_GRID_SEARCH:
    ...
    grid_csv_out = os.path.join(out_root_models, "Regression_grid_search_results(VentTIME)_8.csv")
    df_grid.to_csv(grid_csv_out, index=False)
    print("✅ Grid search summary saved:", grid_csv_out)

    cv_json_out = os.path.join(out_root_models, "Regression_cv_predictions(VentTIME)_8.json")
...
def evaluate_and_save_best_per_model():
    ...
    test_preds_json = os.path.join(out_root_models, "Regression_best_model_test_predictions(VentTIME)_8.json")
    val_preds_json  = os.path.join(out_revision_dir, "Regression_best_model_validation_predictions(VentTIME)_8.json")
    ...
    pd.DataFrame(metrics_rows_test).to_csv(
        os.path.join(out_root_models, "Regression_best_models_TEST_metrics_point(VentTIME)_8.csv"),
        index=False
    )
    pd.DataFrame(metrics_rows_val).to_csv(
        os.path.join(out_revision_dir, "Regression_best_models_VALIDATION_metrics_point(VentTIME)_8.csv"),
        index=False
    )
...
def run_bootstrap_ci(preds_json_path: str, split_name: str, out_dir: str):
    ...
    out_csv = os.path.join(out_dir, f"Regression_BootstrapCI_{split_name}_VentTIME_8.csv")
...
def run_learning_curves():
    ...
    lc_csv = os.path.join(out_revision_dir, "Regression_LearningCurve_VentTIME_8.csv")
