# ======================================================================================
# TBME Major Revision Utilities (REGRESSION: OSI + VentFREQ feature set; NO PCA)
...
save_root = os.path.join(out_root_models, "SavedModels_OSIandVentFREQ_8")  # NEW folder
...
# ---------------------------
# OSI + VentFREQ feature set per TW (TW1..TW6)
# ---------------------------
base_template = [
...
print(f"✅ D_perTW (VentFREQ): {X.shape[2]}")
...
if RUN_GRID_SEARCH:
    ...
    grid_csv_out = os.path.join(out_root_models, "Regression_grid_search_results(OSIandVentFREQ)_8.csv")
    df_grid.to_csv(grid_csv_out, index=False)
    print("✅ Grid search summary saved:", grid_csv_out)

    cv_json_out = os.path.join(out_root_models, "Regression_cv_predictions(OSIandVentFREQ)_8.json")
...
def load_validation_set(scaler_train_x: StandardScaler, scaler_train_y: StandardScaler):
    ...
    if missing:
        raise ValueError(f"Missing {len(missing)} VentFREQ feature cols in {sheet_val}. First 20: {missing[:20]}")
...
def evaluate_and_save_best_per_model():
    ...
    test_preds_json = os.path.join(out_root_models, "Regression_best_model_test_predictions(OSIandVentFREQ)_8.json")
    val_preds_json  = os.path.join(out_revision_dir, "Regression_best_model_validation_predictions(OSIandVentFREQ)_8.json")
    ...
    pd.DataFrame(metrics_rows_test).to_csv(
        os.path.join(out_root_models, "Regression_best_models_TEST_metrics_point(OSIandVentFREQ)_8.csv"),
        index=False
    )
    pd.DataFrame(metrics_rows_val).to_csv(
        os.path.join(out_revision_dir, "Regression_best_models_VALIDATION_metrics_point(OSIandVentFREQ)_8.csv"),
        index=False
    )
...
def run_bootstrap_ci(preds_json_path: str, split_name: str, out_dir: str):
    ...
    out_csv = os.path.join(out_dir, f"Regression_BootstrapCI_{split_name}_OSIandVentFREQ_8.csv")
...
def run_learning_curves():
    ...
    lc_csv = os.path.join(out_revision_dir, "Regression_LearningCurve_OSIandVentFREQ_8.csv")
