# ======================================================================================
# TBME Major Revision Utilities (OSI + VentFREQ non-CNN feature set)
...
#
# NOTE: OSI+VentFREQ (stats + optional wavelet) => NO PCA64. Keeps the rest the same.
# ======================================================================================
...
save_root = os.path.join(out_root_models, "SavedModels_OSIandVentFREQ_8")
...
if RUN_GRID_SEARCH:
    ...
    grid_csv_out = os.path.join(out_root_models, "Classification_grid_search_results(OSIandVentFREQ)_8.csv")
    df_grid.to_csv(grid_csv_out, index=False)
    print("✅ Grid search summary saved:", grid_csv_out)

    cv_json_out = os.path.join(out_root_models, "Classification_cv_predictions(OSIandVentFREQ)_8.json")
...
def evaluate_and_save_best_per_model():
    ...
    test_preds_json = os.path.join(out_root_models, "Classification_best_model_test_predictions(OSIandVentFREQ)_8.json")
    val_preds_json  = os.path.join(out_revision_dir, "Classification_best_model_validation_predictions(OSIandVentFREQ)_8.json")
    ...
    pd.DataFrame(metrics_rows_test).to_csv(
        os.path.join(out_root_models, "Classification_best_models_TEST_metrics_point(OSIandVentFREQ)_8.csv"),
        index=False
    )
    pd.DataFrame(metrics_rows_val).to_csv(
        os.path.join(out_revision_dir, "Classification_best_models_VALIDATION_metrics_point(OSIandVentFREQ)_8.csv"),
        index=False
    )
...
def run_bootstrap_and_calibration(preds_json_path: str, split_name: str, out_dir: str):
    ...
    ci_csv  = os.path.join(out_dir, f"Classification_BootstrapCI_{split_name}_OSIandVentFREQ_8.csv")
    cal_csv = os.path.join(out_dir, f"Classification_CalibrationCurve_{split_name}_OSIandVentFREQ_8.csv")
...
def run_learning_curves():
    ...
    lc_csv = os.path.join(out_revision_dir, "Classification_LearningCurve_OSIandVentFREQ_8.csv")
