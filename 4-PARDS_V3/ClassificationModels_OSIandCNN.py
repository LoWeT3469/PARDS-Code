...
LEARNING_SEEDS     = [0]              # repeats per fraction (3 is enough)
...
def run_bootstrap_and_calibration(preds_json_path: str, split_name: str, out_dir: str):
    ...
    ci_csv  = os.path.join(out_dir, f"Classification_BootstrapCI_{split_name}_OSIandCNN_8.csv")
    cal_csv = os.path.join(out_dir, f"Classification_CalibrationCurve_{split_name}_OSIandCNN_8.csv")
...
def run_learning_curves():
    ...
    lc_csv = os.path.join(out_revision_dir, "Classification_LearningCurve_OSIandCNN_8.csv")
...
def run_pca_sensitivity():
    ...
    pca_csv = os.path.join(out_revision_dir, "Classification_PCASensitivity_OSIandCNN_8.csv")
