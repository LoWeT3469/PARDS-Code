# ======================================================================================
# TBME Major Revision Utilities (CNN feature set example)
...
# - PCA sensitivity (None vs PCA-64 [+ optional PCA-32])
# - Patient-level bootstrap CIs + Sens@fixed specificity + Calibration metrics/curve
#
# ✅ UPDATE (per your request):
#   1) Save training CV predictions JSON (for Train-set ROC/PRC/F1 plots)
#   2) Save std metrics (AUC_std/AUPRC_std/F1_std) in grid CSV (for bar chart)
...
def build_sequences_from_df(df: pd.DataFrame, label_col: str, tw_features: dict, threshold: float):
    X, y, groups = [], [], []
...
X6, y, groups = build_sequences_from_df(df, target_col, tw_features, threshold)
X = add_delta_window(X6)  # (N,7,D)
...
def fit_pca_on_train(X_train_scaled: np.ndarray, n_components: int) -> PCA:
    # PCA across feature dimension D, fit on flattened (N*T, D)
    N, T, D = X_train_scaled.shape
    pca = PCA(n_components=n_components, random_state=0)
    pca.fit(X_train_scaled.reshape(-1, D))
    return pca

def apply_pca(pca: PCA, X_scaled: np.ndarray) -> np.ndarray:
...
def train_one_config_cv(
    ...
    """
    ✅ UPDATED:
      - returns (summary, combo_best_state, cv_pred_records)
      - summary now includes std metrics needed for bar plot
      - cv_pred_records matches your old CV JSON schema for train ROC/PRC/F1 plots
    """
    ...
    for rep in range(n_repeats):
        ...
        for fold, (tr_idx, val_idx) in enumerate(gkf.split(X_sub, y_sub, g_sub)):
            ...
            # Validation metrics + CV-preds record
            model.eval()
            with torch.no_grad():
            ...
            prob2 = prob[valid_mask]
            yt2 = yt[valid_mask]

            if len(np.unique(yt2)) < 2:
                continue

            # metrics
            all_aucs.append(float(roc_auc_score(yt2, prob2)))
            all_auprcs.append(float(auprc_score(yt2, prob2)))
            all_f1s.append(float(f1_score(yt2, (prob2 >= 0.5).astype(int))))

            # save CV predictions (for your train ROC/PRC/F1 plotting)
            ...
            # best fold model for this config by loss
...
# We'll store best-per-model-type based on AUC_mean (complete CV only)
best_per_model = {m: {"auc": -np.inf, "cfg": None, "state": None} for m in model_types}
grid_summary_rows = []

# ✅ UPDATED: store CV predictions across all configs
cv_predictions_all = []
...
if RUN_GRID_SEARCH:
                        ...
                        for lr in [0.001, 0.01]:
                            for wd in [0, 1e-4]:
                                for optimizer_name in ["Adam", "SGD"]:
                                    ...
                                    # ✅ UPDATED: get cv_preds
                                    summary, best_state, cv_preds = train_one_config_cv(
                                        model_type,
                                        X_train_aug, y_train_aug, groups_train_aug,
                                        hidden_dim, dropout, num_layers, batch_size, lr, wd, optimizer_name,
                                        epochs=epochs, n_repeats=n_repeats, n_splits=n_splits
                                    )
                                    # ✅ UPDATED: accumulate CV preds
                                    if cv_preds:
                                        cv_predictions_all.extend(cv_preds)
                                    ...
                                    if np.isfinite(auc_mean) and auc_mean > best_per_model[model_type]["auc"] and summary["has_state"]:
                                        ...
                                        print(f"\n🏆 BEST for {model_type}: AUC_mean={auc_mean:.4f} cfg={best_per_model[model_type]['cfg']}")
    ...
    grid_csv_out = os.path.join(out_root_models, "Classification_grid_search_results(CNN)_8.csv")
    df_grid.to_csv(grid_csv_out, index=False)
    print("✅ Grid search summary saved:", grid_csv_out)

    # ✅ UPDATED: Save CV predictions JSON for train ROC/PRC/F1 plotting
    cv_json_out = os.path.join(out_root_models, "Classification_cv_predictions(CNN)_8.json")
...
def evaluate_and_save_best_per_model():
    ...
    test_preds_json = os.path.join(out_root_models, "Classification_best_model_test_predictions(CNN)_8.json")
    val_preds_json  = os.path.join(out_revision_dir, "Classification_best_model_validation_predictions(CNN)_8.json")
    ...
    pd.DataFrame(metrics_rows_test).to_csv(
        os.path.join(out_root_models, "Classification_best_models_TEST_metrics_point(CNN)_8.csv"),
        index=False
    )
    pd.DataFrame(metrics_rows_val).to_csv(
        os.path.join(out_revision_dir, "Classification_best_models_VALIDATION_metrics_point(CNN)_8.csv"),
        index=False
    )
...
def run_bootstrap_and_calibration(preds_json_path: str, split_name: str, out_dir: str):
    ...
    for item in payloads:
        ...
        # Calibration curve bins (point estimate)
    ...
    ci_csv  = os.path.join(out_dir, f"Classification_BootstrapCI_{split_name}_CNN_8.csv")
    cal_csv = os.path.join(out_dir, f"Classification_CalibrationCurve_{split_name}_CNN_8.csv")
...
def run_learning_curves():
    ...
    lc_csv = os.path.join(out_revision_dir, "Classification_LearningCurve_CNN_8.csv")
...
def run_pca_sensitivity():
    ...
    for model_type in PCA_MODELS:
        ...
        for k in PCA_COMPONENTS_LIST:
            pca = fit_pca_on_train(X_train, n_components=k)

            X_train_pca = apply_pca(pca, X_train)
            X_test_pca  = apply_pca(pca, X_test)
            X_val_pca   = apply_pca(pca, X_val_scaled)
            ...
            print(f"✅ PCA{k} {model_type}: test_AUC={pm_test['AUC']:.3f} val_AUC={pm_val['AUC']:.3f}")

    df_pca = pd.DataFrame(rows)
    pca_csv = os.path.join(out_revision_dir, "Classification_PCASensitivity_CNN_8.csv")
