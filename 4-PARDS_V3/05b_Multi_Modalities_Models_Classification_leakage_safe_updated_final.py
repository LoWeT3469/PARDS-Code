...
for family in MODEL_FAMILIES:
    ...
    for fusion_type in ["early", "cross"]:
        for cfg in PARAM_GRID[family]:
            mean_auc, std_auc = evaluate_cv_teacher(family, fusion_type, cfg, df_train_raw, groups_train)
            teacher_cv_rows.append({"family": family, "fusion": fusion_type, "hidden_dim": cfg["hidden_dim"], "dropout": cfg["dropout"], "num_layers": cfg["num_layers"], "lr": cfg["lr"], "cv_auc_mean": mean_auc, "cv_auc_std": std_auc})
            print(f"[TEACHER][{family}][{fusion_type}] {cfg} -> AUC {mean_auc:.4f} ± {std_auc:.4f}")
            if mean_auc > best_auc:
                best_auc, best_cfg, best_fusion = mean_auc, cfg, fusion_type
    best_teacher[family] = {"cfg": best_cfg, "fusion": best_fusion, "cv_auc": best_auc}
...
inner_tr_idx, inner_va_idx = get_inner_train_val_split(df=df_train, y=y_train_raw, groups=groups_train, val_ratio=0.15, seed=SEED)
...
print("Inner train positives:", int(y_tr.sum()), "/", len(y_tr), "=", float(y_tr.mean()))
print("Inner val positives  :", int(y_va.sum()), "/", len(y_va), "=", float(y_va.mean()))
...
for family in MODEL_FAMILIES:
    ...
    prob_test = predict_baseline(model, Xv_test)
    prob_v3 = predict_baseline(model, Xv_v3_scaled)

    test_m = classification_metrics(y_test_raw, prob_test)
    v3_m = classification_metrics(y_v3, prob_v3)
    ...
    predictions[f"baseline_{family}"] = {"test_true": y_test_raw.tolist(), "test_prob": prob_test.tolist(), "v3_true": y_v3.tolist(), "v3_prob": prob_v3.tolist()}
...
for family in MODEL_FAMILIES:
    ...
    prob_test = predict_teacher(model, Xv_test, Xi_test)
    test_m = classification_metrics(y_test_raw, prob_test)
    results_rows.append({"experiment": "teacher", "family": family, "fusion": fusion, **{f"test_{k}": v for k, v in test_m.items()}})
    predictions[f"teacher_{family}_{fusion}"] = {"test_true": y_test_raw.tolist(), "test_prob": prob_test.tolist()}
...
for family in MODEL_FAMILIES:
    ...
    prob_test = predict_baseline(student, Xv_test)
    prob_v3 = predict_baseline(student, Xv_v3_scaled)
    ...
    predictions[f"student_{family}"] = {"test_true": y_test_raw.tolist(), "test_prob": prob_test.tolist(), "v3_true": y_v3.tolist(), "v3_prob": prob_v3.tolist()}
