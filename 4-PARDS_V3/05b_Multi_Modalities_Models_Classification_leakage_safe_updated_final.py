...
"""
05b_Multi_Modalities_Models_Classification_leakage_safe.py
...
ENCODER_TAG = "vit_base_patch16_224_lora_ft_classification"
...
OUT_DIR = os.path.join(BASE_OUT_DIR, f"MMCls_VentALL_CXR_{ENCODER_TAG}")
...
def sens_at_fixed_spec(y_true, y_prob, spec_target=0.90):
    ...
    for thr in thresholds:
        ...
        if np.isfinite(spec) and spec >= spec_target:
            best_sens = sens
            best_thr = thr
            break
    return float(best_sens) if np.isfinite(best_sens) else np.nan, float(best_thr) if np.isfinite(best_thr) else np.nan
...
def calibration_curve_bins(y_true, y_prob, n_bins=10):
    ...
    for i in range(n_bins):
        ...
        if i == n_bins - 1:
            ...
            mask = (y_prob >= lo) & (y_prob < hi)
        if mask.sum() == 0:
            rows.append({"bin": i + 1, "bin_lo": lo, "bin_hi": hi, "n": 0, "mean_pred": np.nan, "frac_pos": np.nan})
        else:
            rows.append({"bin": i + 1, "bin_lo": lo, "bin_hi": hi, "n": int(mask.sum()), "mean_pred": float(y_prob[mask].mean()), "frac_pos": float(y_true[mask].mean())})
...
def classification_metrics(y_true, y_prob, threshold=0.5):
    ...
    if len(np.unique(y_true)) > 1:
        ...
        out["AUPRC"] = float(auprc_score(y_true, y_prob))
    ...
    for spec_target in SPEC_TARGETS:
        sens, thr = sens_at_fixed_spec(y_true, y_prob, spec_target=spec_target)
        out[f"Sens@Spec{int(spec_target*100)}"] = sens
        out[f"Sens@Spec{int(spec_target*100)}Threshold"] = thr
    return out

def bootstrap_ci_metrics_grouped(y_true, y_prob, groups, B=1000, seed=0):
    y_true = np.asarray(y_true).astype(int)
    y_prob = np.asarray(y_prob).astype(float)
    ...
    for _ in range(B):
        ...
        if len(np.unique(yt)) < 2:
            continue
        metrics.append(classification_metrics(yt, yp))
    rows = []
    if not metrics:
        return pd.DataFrame(columns=["metric", "point", "ci_low", "ci_high", "n_valid_bootstrap"])
    point = classification_metrics(y_true, y_prob)
    for metric in point.keys():
        vals = [m[metric] for m in metrics if metric in m and np.isfinite(m[metric])]
        if not vals:
            continue
        rows.append({
            "metric": metric,
            ...
            "n_valid_bootstrap": int(len(vals)),
        })
    return pd.DataFrame(rows)
...
def train_student(student, teacher, train_loader, val_loader, cfg):
    ...
    for _ in range(EPOCHS):
        ...
        for xv, xi, y in train_loader:
            xv, xi, y = xv.to(DEVICE), xi.to(DEVICE), y.to(DEVICE)
            with torch.no_grad():
                t_logits = teacher(xv, xi)
            s_logits = student(xv)
            hard_loss = bce(s_logits, y)
            soft_loss = mse(s_logits, t_logits)
...
def predict_baseline(model, Xv):
    ...
    with torch.no_grad():
        ...
        logits = model(xv).detach().cpu().numpy().ravel()
...
def predict_teacher(model, Xv, Xi):
    ...
    with torch.no_grad():
        ...
        logits = model(xv, xi).detach().cpu().numpy().ravel()
    return 1.0 / (1.0 + np.exp(-logits))
...
try:
    ...
    print("WARNING: StratifiedGroupKFold not available. Falling back to non-stratified grouped splits.")

def get_cv_splitter(n_splits=5, seed=3469):
...
def get_inner_train_val_split(df, y, groups, val_ratio=0.15, seed=3469):
    if val_ratio <= 0:
        ...
        return idx, idx
    if HAS_STRATIFIED_GROUP_KFOLD:
        n_splits = int(round(1 / val_ratio))
...
def save_classification_posthoc(prefix, y_true, y_prob, groups, out_dir):
    metrics_df = pd.DataFrame([classification_metrics(y_true, y_prob)])
    metrics_df.to_csv(os.path.join(out_dir, f"{prefix}_point_metrics.csv"), index=False)
    if RUN_BOOTSTRAP_CI_EVAL:
        ci_df = bootstrap_ci_metrics_grouped(y_true, y_prob, groups, B=BOOTSTRAP_B, seed=SEED)
        ci_df.to_csv(os.path.join(out_dir, f"{prefix}_bootstrap_ci.csv"), index=False)
    cal_df = calibration_curve_bins(y_true, y_prob, n_bins=CALIB_BINS)
    cal_df.to_csv(os.path.join(out_dir, f"{prefix}_calibration_curve.csv"), index=False)

def train_eval_single_run_classification(experiment_type, family, run_seed, reducer_cfg=None, distill_cfg=None, fraction=1.0):
    set_seed(run_seed)
    idx_all = np.arange(len(df_train))
    if fraction < 1.0:
        rng = np.random.RandomState(run_seed)
        sample_n = max(8, int(round(len(idx_all) * fraction)))
        chosen = rng.choice(idx_all, size=sample_n, replace=False)
    ...
    y_sub = (df_train_sub[target_train].to_numpy(dtype=np.float32) >= CLASS_THRESHOLD).astype(np.float32)
    inner_tr_idx, inner_va_idx = get_inner_train_val_split(df=df_train_sub, y=y_sub, groups=groups_sub, val_ratio=FINAL_VAL_RATIO if FINAL_VAL_STRATEGY == "holdout" else 0.15, seed=run_seed)
    Xv_tr, Xv_va = Xv_sub[inner_tr_idx], Xv_sub[inner_va_idx]
    y_tr = y_sub[inner_tr_idx].reshape(-1, 1).astype(np.float32)
    y_va = y_sub[inner_va_idx].reshape(-1, 1).astype(np.float32)
    ...
    if experiment_type == "baseline":
        ...
        return predict_baseline(model, Xv_te), predict_baseline(model, Xv_v3_sub)
    Xi_sub_raw = df_train_sub[cxr_emb_cols].to_numpy(dtype=np.float32)
    img_scaler, _, [Xi_sub, Xi_te] = fit_cxr_transform_train_only(Xi_sub_raw, [Xi_test_raw], reducer_cfg=reducer_cfg)
    Xi_v3_dummy = None
    Xv_tr, Xv_va = Xv_sub[inner_tr_idx], Xv_sub[inner_va_idx]
    ...
    if experiment_type == "teacher":
        ...
        return predict_teacher(model, Xv_te, Xi_te), None
    ...
    return predict_baseline(student_model, Xv_te), predict_baseline(student_model, Xv_v3_sub)
...
y_train_bin_all = (df_train_raw[target_train].to_numpy(dtype=np.float32) >= CLASS_THRESHOLD).astype(np.float32)
y_test_bin_all = (df_test_raw[target_train].to_numpy(dtype=np.float32) >= CLASS_THRESHOLD).astype(np.float32)
y_v3_bin_all = (df_v3[target_val].to_numpy(dtype=np.float32) >= CLASS_THRESHOLD).astype(np.float32)
...
print("Train positives:", int(y_train_bin_all.sum()), "/", len(y_train_bin_all), "=", float(y_train_bin_all.mean()))
print("Test positives :", int(y_test_bin_all.sum()), "/", len(y_test_bin_all), "=", float(y_test_bin_all.mean()))
print("V3 positives   :", int(y_v3_bin_all.sum()), "/", len(y_v3_bin_all), "=", float(y_v3_bin_all.mean()))
...
def prepare_fold_data(df_tr_raw, df_va_raw):
    ...
    y_tr = (df_tr[target_train].to_numpy(dtype=np.float32) >= CLASS_THRESHOLD).astype(np.float32).reshape(-1, 1)
    y_va = (df_va[target_train].to_numpy(dtype=np.float32) >= CLASS_THRESHOLD).astype(np.float32).reshape(-1, 1)

    return {"Xv_tr": Xv_tr, "Xv_va": Xv_va, "Xi_tr": Xi_tr, "Xi_va": Xi_va, "y_tr": y_tr, "y_va": y_va, "y_va_flat": y_va.ravel()}

def evaluate_cv_baseline(family, cfg, df_train_raw, groups_train):
    splitter = get_cv_splitter(n_splits=N_FOLDS, seed=SEED)
    fold_aucs = []
    dummy_y = (df_train_raw[target_train].to_numpy(dtype=np.float32) >= CLASS_THRESHOLD).astype(np.float32)
    for tr_i, va_i in splitter.split(df_train_raw, dummy_y, groups=groups_train):
        ...
        prob = predict_baseline(result.model, prep["Xv_va"])
        truth = prep["y_va_flat"].astype(int)
        if len(np.unique(truth)) > 1:
            fold_aucs.append(float(roc_auc_score(truth, prob)))
...
def evaluate_cv_teacher(family, fusion_type, cfg, reducer_cfg, df_train_raw, groups_train):
    ...
    for tr_i, va_i in splitter.split(df_train_raw, dummy_y, groups=groups_train):
        ...
        prob = predict_teacher(result.model, prep["Xv_va"], prep["Xi_va"])
        truth = prep["y_va_flat"].astype(int)
        if len(np.unique(truth)) > 1:
            fold_aucs.append(float(roc_auc_score(truth, prob)))
    return float(np.mean(fold_aucs)), float(np.std(fold_aucs))
...
def evaluate_cv_student(family, fusion_type, teacher_cfg, student_cfg, reducer_cfg, distill_cfg, df_train_raw, groups_train):
    ...
    for tr_i, va_i in splitter.split(df_train_raw, dummy_y, groups=groups_train):
        ...
        prob = predict_baseline(student, prep["Xv_va"])
        truth = prep["y_va_flat"].astype(int)
        if len(np.unique(truth)) > 1:
            fold_aucs.append(float(roc_auc_score(truth, prob)))
    return float(np.mean(fold_aucs)), float(np.std(fold_aucs))
...
for family in MODEL_FAMILIES:
    best_auc, best_cfg = -np.inf, None
    for cfg in PARAM_GRID[family]:
        mean_auc, std_auc = evaluate_cv_baseline(family, cfg, df_train_raw, groups_train)
        baseline_cv_rows.append({"family": family, "hidden_dim": cfg["hidden_dim"], "dropout": cfg["dropout"], "num_layers": cfg["num_layers"], "lr": cfg["lr"], "cv_auc_mean": mean_auc, "cv_auc_std": std_auc})
        print(f"[BASELINE][{family}] {cfg} -> AUC {mean_auc:.4f} ± {std_auc:.4f}")
        if mean_auc > best_auc:
            best_auc, best_cfg = mean_auc, cfg
    best_baseline[family] = {"cfg": best_cfg, "cv_auc": best_auc}
pd.DataFrame(baseline_cv_rows).to_csv(os.path.join(OUT_DIR, "baseline_cv_results.csv"), index=False)

teacher_cv_rows = []
best_teacher = {}
for family in MODEL_FAMILIES:
    best_auc, best_cfg, best_fusion = -np.inf, None, None
    for fusion_type in ["early", "cross"]:
        for reducer_cfg in CXR_REDUCER_CANDIDATES:
            for cfg in PARAM_GRID[family]:
                mean_auc, std_auc = evaluate_cv_teacher(family, fusion_type, cfg, reducer_cfg, df_train_raw, groups_train)
                teacher_cv_rows.append({"family": family, "fusion": fusion_type, "cxr_reducer": reducer_cfg["name"], "cxr_latent_dim": reducer_cfg.get("latent_dim"), "hidden_dim": cfg["hidden_dim"], "dropout": cfg["dropout"], "num_layers": cfg["num_layers"], "lr": cfg["lr"], "weight_decay": cfg.get("weight_decay"), "bidirectional": cfg.get("bidirectional", False), "cv_auc_mean": mean_auc, "cv_auc_std": std_auc})
                print(f"[TEACHER][{family}][{fusion_type}][{reducer_cfg['name']}] {cfg} -> AUC {mean_auc:.4f} ± {std_auc:.4f}")
                if mean_auc > best_auc:
                    best_auc, best_cfg, best_fusion = mean_auc, cfg, fusion_type
                    best_teacher[family] = {"cfg": best_cfg, "fusion": best_fusion, "reducer_cfg": reducer_cfg, "cv_auc": best_auc}
...
for family in MODEL_FAMILIES:
    ...
    best_auc, best_cfg, best_distill = -np.inf, None, None
    for cfg in PARAM_GRID[family]:
        for distill_cfg in DISTILLATION_CANDIDATES:
            mean_auc, std_auc = evaluate_cv_student(family, teacher_fusion, teacher_cfg, cfg, reducer_cfg, distill_cfg, df_train_raw, groups_train)
            student_cv_rows.append({"family": family, "teacher_fusion": teacher_fusion, "cxr_reducer": reducer_cfg["name"], "student_hidden_dim": cfg["hidden_dim"], "student_dropout": cfg["dropout"], "student_num_layers": cfg["num_layers"], "student_lr": cfg["lr"], "student_weight_decay": cfg.get("weight_decay"), "student_bidirectional": cfg.get("bidirectional", False), "alpha": distill_cfg["alpha"], "beta": distill_cfg["beta"], "cv_auc_mean": mean_auc, "cv_auc_std": std_auc})
            print(f"[STUDENT][{family}][alpha={distill_cfg['alpha']:.2f},beta={distill_cfg['beta']:.2f}] {cfg} -> AUC {mean_auc:.4f} ± {std_auc:.4f}")
            if mean_auc > best_auc:
                best_auc, best_cfg, best_distill = mean_auc, cfg, distill_cfg
    best_student[family] = {"cfg": best_cfg, "distill_cfg": best_distill, "cv_auc": best_auc}
...
y_train_raw = (df_train[target_train].to_numpy(dtype=np.float32) >= CLASS_THRESHOLD).astype(np.float32)
y_test_raw = (df_test[target_train].to_numpy(dtype=np.float32) >= CLASS_THRESHOLD).astype(np.float32)
y_v3 = (df_v3_imp[target_val].to_numpy(dtype=np.float32) >= CLASS_THRESHOLD).astype(np.float32)
...
baseline_family_probs = {}
teacher_family_probs = {}
student_family_probs = {}
...
for family in MODEL_FAMILIES:
    ...
    baseline_test_probs, baseline_v3_probs = [], []
    teacher_test_probs, student_test_probs, student_v3_probs = [], [], []
    ...
    for run_seed in FINAL_TRAIN_SEEDS:
        ...
        if FINAL_VAL_STRATEGY == "holdout":
            ...
            y_tr = y_train_raw[inner_tr_idx].reshape(-1, 1).astype(np.float32)
            y_va = y_train_raw[inner_va_idx].reshape(-1, 1).astype(np.float32)
            baseline_va_loader = DataLoader(VentOnlyDataset(Xv_va, y_va), batch_size=BATCH_SIZE, shuffle=False)
            multi_va_loader = DataLoader(MultiDataset(Xv_va, Xi_va, y_va), batch_size=BATCH_SIZE, shuffle=False)
        else:
            Xv_tr, Xi_tr = Xv_train, Xi_train
            y_tr = y_train_raw.reshape(-1, 1).astype(np.float32)
        ...
        baseline_test_probs.append(predict_baseline(baseline_model, Xv_test))
        baseline_v3_probs.append(predict_baseline(baseline_model, Xv_v3_scaled))
        teacher_test_probs.append(predict_teacher(teacher_model, Xv_test, Xi_test))
        student_test_probs.append(predict_baseline(student_model, Xv_test))
        student_v3_probs.append(predict_baseline(student_model, Xv_v3_scaled))

    baseline_test = mean_stack(baseline_test_probs)
    baseline_v3 = mean_stack(baseline_v3_probs)
    teacher_test = mean_stack(teacher_test_probs)
    student_test = mean_stack(student_test_probs)
    student_v3 = mean_stack(student_v3_probs)
    baseline_family_probs[family] = {"test": baseline_test, "v3": baseline_v3}
    teacher_family_probs[family] = {"test": teacher_test}
    student_family_probs[family] = {"test": student_test, "v3": student_v3}

    test_m = classification_metrics(y_test_raw, baseline_test)
    v3_m = classification_metrics(y_v3, baseline_v3)
    ...
    predictions[f"baseline_{family}"] = {"test_true": y_test_raw.tolist(), "test_prob": baseline_test.tolist(), "v3_true": y_v3.tolist(), "v3_prob": baseline_v3.tolist()}

    test_m = classification_metrics(y_test_raw, teacher_test)
    ...
    predictions[f"teacher_{family}_{teacher_choice['fusion']}"] = {"test_true": y_test_raw.tolist(), "test_prob": teacher_test.tolist()}

    test_m = classification_metrics(y_test_raw, student_test)
    v3_m = classification_metrics(y_v3, student_v3)
    ...
    predictions[f"student_{family}"] = {"test_true": y_test_raw.tolist(), "test_prob": student_test.tolist(), "v3_true": y_v3.tolist(), "v3_prob": student_v3.tolist()}

baseline_top_families = sorted(MODEL_FAMILIES, key=lambda fam: best_baseline[fam]["cv_auc"], reverse=True)[:ENSEMBLE_TOP_K]
baseline_test_ensemble = mean_stack([baseline_family_probs[fam]["test"] for fam in baseline_top_families])
baseline_v3_ensemble = mean_stack([baseline_family_probs[fam]["v3"] for fam in baseline_top_families])
results_rows.append({"experiment": "baseline_ensemble", "family": "+".join(baseline_top_families), "fusion": "vent_only", "ensemble_type": "family", "ensemble_size": len(baseline_top_families), "train_seeds": len(FINAL_TRAIN_SEEDS), **{f"test_{k}": v for k, v in classification_metrics(y_test_raw, baseline_test_ensemble).items()}, **{f"v3_{k}": v for k, v in classification_metrics(y_v3, baseline_v3_ensemble).items()}})
predictions["baseline_family_ensemble"] = {"members": baseline_top_families, "test_true": y_test_raw.tolist(), "test_prob": baseline_test_ensemble.tolist(), "v3_true": y_v3.tolist(), "v3_prob": baseline_v3_ensemble.tolist()}
ensemble_members["baseline_family_ensemble"] = baseline_top_families

teacher_top_families = sorted(MODEL_FAMILIES, key=lambda fam: best_teacher[fam]["cv_auc"], reverse=True)[:ENSEMBLE_TOP_K]
teacher_test_ensemble = mean_stack([teacher_family_probs[fam]["test"] for fam in teacher_top_families])
results_rows.append({"experiment": "teacher_ensemble", "family": "+".join(teacher_top_families), "fusion": "multimodal", "ensemble_type": "family", "ensemble_size": len(teacher_top_families), "train_seeds": len(FINAL_TRAIN_SEEDS), **{f"test_{k}": v for k, v in classification_metrics(y_test_raw, teacher_test_ensemble).items()}})
predictions["teacher_family_ensemble"] = {"members": teacher_top_families, "test_true": y_test_raw.tolist(), "test_prob": teacher_test_ensemble.tolist()}
ensemble_members["teacher_family_ensemble"] = teacher_top_families

student_top_families = sorted(MODEL_FAMILIES, key=lambda fam: best_student[fam]["cv_auc"], reverse=True)[:ENSEMBLE_TOP_K]
student_test_ensemble = mean_stack([student_family_probs[fam]["test"] for fam in student_top_families])
student_v3_ensemble = mean_stack([student_family_probs[fam]["v3"] for fam in student_top_families])
results_rows.append({"experiment": "student_ensemble", "family": "+".join(student_top_families), "fusion": "vent_only_distilled", "ensemble_type": "family", "ensemble_size": len(student_top_families), "train_seeds": len(FINAL_TRAIN_SEEDS), **{f"test_{k}": v for k, v in classification_metrics(y_test_raw, student_test_ensemble).items()}, **{f"v3_{k}": v for k, v in classification_metrics(y_v3, student_v3_ensemble).items()}})
predictions["student_family_ensemble"] = {"members": student_top_families, "test_true": y_test_raw.tolist(), "test_prob": student_test_ensemble.tolist(), "v3_true": y_v3.tolist(), "v3_prob": student_v3_ensemble.tolist()}
...
for key, payload in predictions.items():
    if "test_prob" in payload:
        save_classification_posthoc(f"{key}_test", np.asarray(payload["test_true"]), np.asarray(payload["test_prob"]), test_groups, posthoc_dir)
    if "v3_prob" in payload:
        save_classification_posthoc(f"{key}_v3", np.asarray(payload["v3_true"]), np.asarray(payload["v3_prob"]), v3_groups, posthoc_dir)

if RUN_LEARNING_CURVES:
    learning_rows = []
    learning_specs = [
        ("baseline", max(best_baseline, key=lambda fam: best_baseline[fam]["cv_auc"])),
        ("teacher", max(best_teacher, key=lambda fam: best_teacher[fam]["cv_auc"])),
        ("student", max(best_student, key=lambda fam: best_student[fam]["cv_auc"])),
    ]
    for experiment_type, family in learning_specs:
        ...
        for frac in LEARNING_FRACTIONS:
            for learn_seed in LEARNING_SEEDS:
                test_prob, v3_prob = train_eval_single_run_classification(experiment_type, family, learn_seed, reducer_cfg=reducer_cfg, distill_cfg=distill_cfg, fraction=frac)
                row = {"experiment": experiment_type, "family": family, "train_fraction": frac, "seed": learn_seed, **{f"test_{k}": v for k, v in classification_metrics(y_test_raw, test_prob).items()}}
                if v3_prob is not None:
                    row.update({f"v3_{k}": v for k, v in classification_metrics(y_v3, v3_prob).items()})
...
if RUN_CXR_REDUCER_SENSITIVITY:
    sensitivity_rows = []
    teacher_family = max(best_teacher, key=lambda fam: best_teacher[fam]["cv_auc"])
    student_family = max(best_student, key=lambda fam: best_student[fam]["cv_auc"])
    for reducer_cfg in CXR_REDUCER_SENSITIVITY_CANDIDATES:
        test_prob, _ = train_eval_single_run_classification("teacher", teacher_family, SEED, reducer_cfg=reducer_cfg, fraction=1.0)
        sensitivity_rows.append({"experiment": "teacher", "family": teacher_family, "reducer": reducer_cfg["name"], "latent_dim": reducer_cfg.get("latent_dim"), **{f"test_{k}": v for k, v in classification_metrics(y_test_raw, test_prob).items()}})
        test_prob, v3_prob = train_eval_single_run_classification("student", student_family, SEED, reducer_cfg=reducer_cfg, distill_cfg=best_student[student_family]["distill_cfg"], fraction=1.0)
        sensitivity_rows.append({"experiment": "student", "family": student_family, "reducer": reducer_cfg["name"], "latent_dim": reducer_cfg.get("latent_dim"), **{f"test_{k}": v for k, v in classification_metrics(y_test_raw, test_prob).items()}, **({f"v3_{k}": v for k, v in classification_metrics(y_v3, v3_prob).items()} if v3_prob is not None else {})})
