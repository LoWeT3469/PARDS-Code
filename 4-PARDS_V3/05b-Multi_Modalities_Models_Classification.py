...
from sklearn.metrics import (
    auc, brier_score_loss, f1_score, precision_recall_curve, roc_auc_score
)
...
OUT_DIR = "/nfs/turbo/med-kayvan-lab/Projects/PARDS/04-Results/PARDS_Risk_V3/TRAM_Classification_VentALL_CXR"
...
# =========================
# Optional stratified grouped splitters
# =========================
try:
...
# Split helpers
# =========================
def get_outer_train_test_split(df, y, groups, test_size=0.2, seed=42):
...
def get_cv_splitter(n_splits=5, seed=42):
    if HAS_STRATIFIED_GROUP_KFOLD:
        return StratifiedGroupKFold(n_splits=n_splits, shuffle=True, random_state=seed)
    else:
        return GroupKFold(n_splits=n_splits)

def get_inner_train_val_split(df, y, groups, val_ratio=0.15, seed=42):
    if HAS_STRATIFIED_GROUP_KFOLD:
...
y_multi = (df_multi[target_train].to_numpy(dtype=np.float32) >= CLASS_THRESHOLD).astype(np.float32)
y_v3 = (df_v3[target_val].to_numpy(dtype=np.float32) >= CLASS_THRESHOLD).astype(np.float32)
...
train_idx, test_idx = get_outer_train_test_split(
    df=df_multi,
    y=y_multi,
    groups=groups_multi,
    test_size=TEST_SIZE,
...
print("\nClass balance:")
print("Full V1V2 positives:", int(y_multi.sum()), "/", len(y_multi), "=", float(y_multi.mean()))
print(
    "Train positives    :",
    int((df_train_raw[target_train].to_numpy(dtype=np.float32) >= CLASS_THRESHOLD).sum()),
    "/",
    len(df_train_raw),
    "=",
    float((df_train_raw[target_train].to_numpy(dtype=np.float32) >= CLASS_THRESHOLD).mean())
)
print(
    "Test positives     :",
    int((df_test_raw[target_train].to_numpy(dtype=np.float32) >= CLASS_THRESHOLD).sum()),
    "/",
    len(df_test_raw),
    "=",
    float((df_test_raw[target_train].to_numpy(dtype=np.float32) >= CLASS_THRESHOLD).mean())
)
print("V3 positives       :", int(y_v3.sum()), "/", len(y_v3), "=", float(y_v3.mean()))
...
inner_tr_idx, inner_va_idx = get_inner_train_val_split(
    df=df_train,
    y=y_train_raw,
    groups=groups_train,
    val_ratio=0.15,
...
for family in MODEL_FAMILIES:
    ...
    predictions[f"baseline_{family}"] = {
        ...
        "test_prob": prob_test.tolist(),
        "v3_true": y_v3.tolist(),
        "v3_prob": prob_v3.tolist(),
    }
